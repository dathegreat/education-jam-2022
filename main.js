...
const tweenVectors = (a, b, steps) => {
  ...
  for (let i = 0; i <= steps; i++) {
    tweenedVectors.push(
      new Vector(
        [Math.floor(a.x + stepSizeX * i), Math.floor(a.y + stepSizeY * i)],
        a.rgba
      )
    );
  }
...
const createTransformFrames = async () => {
  ...
  for (let i = 0; i <= steps; i++) {
    setTimeout(async () => {
      ...
      tweenedFrames[i] = await transformMatrix(tweenedTransform, canvasMatrix);
...
const drawImageFromFile = (source) => {
  ...
  image.src = source;
  image.onload = () => {
    const scaledWidth = Math.floor((image.naturalWidth / canvasSize) * scale);
    const scaledHeight = Math.floor((image.naturalHeight / canvasSize) * scale);
    const center = [
...
const grid = new Matrix([new Vector([scale / 10, 0]), new Vector([0, scale / 10])]);
